-- Script corregido para arreglar ejercicios huérfanos
-- Este script usa solo las columnas que realmente existen en las tablas
...
-- 2. Crear la actividad faltante (id=59) con solo las columnas existentes
-- Primero necesitamos ver qué columnas tiene activity_enrollments
-- Asumiendo que tiene: id, client_id, start_date, end_date, status, created_at, updated_at
...
SELECT 
  ...
  '2024-09-30'::date as end_date,    -- Fecha de fin estimada
  'active' as status,
  NOW() as created_at,
  NOW() as updated_at
WHERE NOT EXISTS (
