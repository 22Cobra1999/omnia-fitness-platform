-- Script de migración seguro con validación de datos
-- Este script valida los datos antes de migrar y maneja referencias huérfanas
...
-- 4. Verificar datos huérfanos antes de migrar
SELECT 
  'Ejercicios con activity_id huérfanos' as status,
  COUNT(*) as count
FROM fitness_exercises fe
LEFT JOIN activity_enrollments ae ON fe.activity_id = ae.id
WHERE ae.id IS NULL;

-- 5. Mostrar ejemplos de datos huérfanos
SELECT 
  fe.id,
  fe.activity_id,
  fe.nombre_actividad,
  'ACTIVIDAD NO EXISTE' as status
FROM fitness_exercises fe
LEFT JOIN activity_enrollments ae ON fe.activity_id = ae.id
WHERE ae.id IS NULL
LIMIT 10;

-- 6. Migrar solo datos válidos (con activity_id existente)
...
SELECT 
  'Datos migrados exitosamente' as status,
...
-- 9. Verificar ejercicios que no se pudieron migrar
SELECT 
  'Ejercicios no migrados (datos huérfanos)' as status,
  COUNT(*) as count
FROM fitness_exercises fe
LEFT JOIN activity_calendar ac ON fe.id = ac.fitness_exercise_id
LEFT JOIN activity_enrollments ae ON fe.activity_id = ae.id
WHERE ac.id IS NULL AND ae.id IS NULL;

-- 10. Mostrar ejercicios huérfanos que no se pudieron migrar
SELECT 
  fe.id,
  fe.activity_id,
  fe.nombre_actividad,
  'NO MIGRADO - ACTIVIDAD NO EXISTE' as status
FROM fitness_exercises fe
LEFT JOIN activity_calendar ac ON fe.id = ac.fitness_exercise_id
LEFT JOIN activity_enrollments ae ON fe.activity_id = ae.id
WHERE ac.id IS NULL AND ae.id IS NULL
