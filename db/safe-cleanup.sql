...
-- =====================================================
-- Este script solo elimina tablas obsoletas si están vacías

-- =====================================================
-- PASO 1: VERIFICACIÓN FINAL DE DATOS
-- =====================================================
...
DECLARE
    ...
    v_total_obsolete_data INTEGER := 0;
BEGIN
    RAISE NOTICE '=========================================';
    RAISE NOTICE 'VERIFICACIÓN FINAL ANTES DE LIMPIAR';
    RAISE NOTICE '=========================================';
    ...
    -- Calcular total de datos obsoletos
    v_total_obsolete_data := v_client_progress_count + v_intensity_levels_count + 
                            v_fitness_exercises_count + v_fitness_details_count + 
                            v_activity_calendar_count + v_customizations_count;
    
    RAISE NOTICE '=========================================';
    RAISE NOTICE 'TOTAL DE DATOS OBSOLETOS: %', v_total_obsolete_data;
    RAISE NOTICE '=========================================';
    
    -- Solo proceder si no hay datos
    IF v_total_obsolete_data > 0 THEN
        RAISE EXCEPTION '❌ HAY % REGISTROS EN TABLAS OBSOLETAS. Ejecutar migración primero: db/migrate-existing-data.sql', v_total_obsolete_data;
    END IF;
