-- ================================================================
-- FINAL NUKE & REBUILD: CLEAN UP TOTAL_SESSIONS ERROR
-- ================================================================

-- 1. DROP PROBLEMATIC TRIGGERS (ELIMINAR TRIGGERS QUE PUEDAN ESTAR FALLANDO)
DO $$
BEGIN
    -- Triggers en planificacion_ejercicios
    DROP TRIGGER IF EXISTS trg_sync_activity_stats ON planificacion_ejercicios;
    DROP TRIGGER IF EXISTS sync_activity_stats_trigger ON planificacion_ejercicios;
    DROP TRIGGER IF EXISTS trg_sync_stats_planificacion ON planificacion_ejercicios;
    
    -- Triggers en periodos
    DROP TRIGGER IF EXISTS trg_sync_stats_periodos ON periodos;
    DROP TRIGGER IF EXISTS sync_periodos_stats_trigger ON periodos;
END $$;

-- 2. ASEGURAR COLUMNAS EN ACTIVITIES (POR SI ACASO)
DO $$
BEGIN
    ...
    -- Asegurar sesiones_dias_totales
...
-- 3. FUNCIÓN DE CÁLCULO DEFINITIVA (USANDO COLUMNAS CORRECTAS)
...
BEGIN
    ...
    IF v_type = 'program' THEN
        -- Items únicos (ejercicios/platos)
...
-- 4. RE-INSTALAR FUNCIÓN DE TRIGGER
...
-- 5. RE-INSTALAR TRIGGERS LIMPIOS
CREATE TRIGGER trg_sync_stats_planificacion 
AFTER INSERT OR UPDATE OR DELETE ON planificacion_ejercicios 
FOR EACH ROW EXECUTE FUNCTION trigger_sync_activity_stats();

CREATE TRIGGER trg_sync_stats_periodos 
AFTER INSERT OR UPDATE OR DELETE ON periodos 
FOR EACH ROW EXECUTE FUNCTION trigger_sync_activity_stats();

-- 6. SINCRONIZACIÓN INICIAL MASIVA
DO $$
...
BEGIN
    FOR act_record IN SELECT id FROM activities LOOP
        ...
        EXCEPTION WHEN OTHERS THEN
            RAISE NOTICE 'Error al sincronizar actividad %: %', act_record.id, SQLERRM;
        END;
    END LOOP;
END $$;
