...
export const MeetDetailActions: React.FC<MeetDetailActionsProps> = ({
    ...
    return (
        <div className="pt-2 flex flex-col gap-2">
            {(isCancelled || isPast) && !pendingReschedule && (
                <div className="text-[10px] text-white/20 text-center uppercase tracking-widest bg-white/[0.02] py-2 rounded-lg border border-white/5">
                    {isCancelled ? 'Reunión cancelada' : 'Reunión finalizada'}
                </div>
            )}
            ...
            {!(isCancelled || isPast) && (
                <>
                    {pendingReschedule && pendingReschedule.status === 'pending' ? (
                        ...
                        <>
                            {/* Branch 1: Unconfirmed Meet (Invitation Stage) */}
                            {!isConfirmed && !isCancelled && !isPast && (
                                <div className="flex flex-col gap-2">
                                    ...
                                    >
                                        {isSentByMe ? 'Cancelar invitación' : 'Rechazar invitación'}
                                    </button>
                                    {!isSentByMe && myRsvp === 'pending' && (
                                        <button
                                            type="button"
                                            disabled={selectedMeetRsvpLoading || !canEditRsvp}
