...
interface CalendarMoveActivitiesModalProps {
    open: boolean
...
export function CalendarMoveActivitiesModal({
    open,
    onOpenChange,
    sourceDate,
    targetDate,
    ...
    isUpdating,
    onCancel
}: CalendarMoveActivitiesModalProps) {
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="bg-[#1A1C1F] border-zinc-800 text-white w-[90%] max-w-sm rounded-2xl">
                ...
                <DialogFooter className="flex gap-2 sm:justify-end mt-2">
                    <Button variant="ghost" onClick={onCancel} className="flex-1 text-gray-400 hover:text-white">
                        Cancelar
                    </Button>
                    <Button
