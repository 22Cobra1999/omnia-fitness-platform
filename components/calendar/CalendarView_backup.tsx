                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="w-full flex justify-between items-start">
                        ...
                        {/* Indicadores compactos para mobile */}
                        <div className="flex sm:hidden gap-1">
                          {hasClientMeet && <div className="w-1.5 h-1.5 rounded-full bg-[#FF7939]"></div>}
                          {busyMinutes > 0 && <div className="w-1.5 h-1.5 rounded-full bg-white/40"></div>}
                        </div>
                      </div>

                      {/* Nombres de actividades en Desktop */}
                      <div className="hidden sm:flex flex-col gap-1 mt-2 w-full overflow-hidden">
                        {activities.slice(0, 3).map((act: any) => (
                          <div key={act.id} className="text-[10px] truncate text-white/60 text-left px-1.5 py-0.5 rounded bg-white/5 border border-white/5">
                            {act.title}
                          </div>
                        ))}
                        {activities.length > 3 && (
                          <div className="text-[9px] text-white/40 text-left px-1.5 mt-0.5">
                            + {activities.length - 3} más
                          </div>
                        )}

                        {meets.slice(0, 2).map((m: any) => {
                          const isWorkshop = m.event_type === 'workshop';
                          const isRed = m.status === 'cancelled' || m.rsvp_status === 'declined';
                          return (
                            <div
                              key={m.id}
                              className={`text-[10px] truncate text-left px-1.5 py-0.5 rounded flex items-center gap-1 border
                                ${isRed ? 'bg-red-500/10 border-red-500/20 text-red-400' : 'bg-[#FF7939]/10 border-[#FF7939]/20 text-[#FFB366]'}
                              `}
                            >
                              {isWorkshop ? <GraduationCap className="w-2.5 h-2.5" /> : <Video className="w-2.5 h-2.5" />}
                              <span className="truncate">{m.title || 'Meet'}</span>
                            </div>
                          );
                        })}
                      </div>

                      {/* Mobile Bubbles (Original UI but slightly adjusted) */}
                      <div className="sm:hidden mt-auto w-full flex flex-col gap-1 items-center">
                        {busyMinutes > 0 && (
                          <div className="flex items-center justify-center scale-90">
                            <span
                              className={
                                `inline-flex items-center justify-center px-1.5 py-0.5 rounded-full border text-[9px] font-bold ` +
                            ...
                            <span
                              className={
                                hasPendingClientMeet
                                  ? 'inline-flex items-center justify-center px-1 h-5 rounded-full border border-[#FF3B30]/40 bg-[#FF3B30]/15 gap-0.5'
                                  : 'inline-flex items-center justify-center px-1 h-5 rounded-full border border-white/20 bg-white/10 gap-0.5'
                              }
                            >
                              {(() => {
                                const hasWorkshop = meets.some(m => m.event_type === 'workshop');
                                const hasCancelled = meets.some(m => m.status === 'cancelled');
                                const iconClass = hasPendingClientMeet ? 'w-2.5 h-2.5 text-[#FF3B30]' : (hasCancelled ? 'w-2.5 h-2.5 text-red-500' : 'w-2.5 h-2.5 text-white');

                                if (hasWorkshop) return <GraduationCap className={iconClass} />;
                                return <Video className={iconClass} />;
                              })()}
                              {meets.length > 1 && (
                                <span className={`text-[8px] font-bold ${hasPendingClientMeet ? 'text-[#FF3B30]' : 'text-white'}`}>
                                  {meets.length}
                                </span>
                              )}
                            </span>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3 min-w-0">
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${isCancelled ? 'bg-red-500/10 text-red-400 border border-red-500/30' : (isPending ? 'bg-[#FF7939]/10 text-[#FF7939] border border-[#FF7939]/20' : 'bg-white/5 text-white/70 border border-white/10')}`}>
                            ...
                            <div className="min-w-0">
                              ...
                              <div className="flex items-center gap-2 mt-0.5">
                                ...
                                {isCancelled && <span className="text-[9px] font-bold uppercase text-red-500 px-1.5 py-0.5 rounded bg-red-500/10 border border-red-500/20">{m.status === 'cancelled' ? 'Cancelada' : 'Rechazada'}</span>}
                              </div>
                            </div>
                          </div>
                          <button
