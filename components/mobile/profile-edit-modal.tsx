...
export function ProfileEditModal({ isOpen, onClose, editingSection }: ProfileEditModalProps) {
  ...
  const hasChanges = () => {
    ...
    const fields = ['full_name', 'email', 'phone', 'location', 'emergency_contact', 'birth_date', 'weight', 'height', 'gender', 'level']
    for (const field of fields) {
      if ((profileData as any)[field] !== (initialData as any)[field]) return true
    }
  ...
  const handleSaveProfile = async () => {
    console.log('💾 [ProfileEditModal] Iniciando guardado de perfil...')
    try {
      ...
      if (newErrors.weight || newErrors.height || newErrors.emergency_contact) {
        setErrors(newErrors)
        console.warn('⚠️ [ProfileEditModal] Datos inválidos:', newErrors)
        toast({
      ...
      console.log('📤 [ProfileEditModal] Enviando datos a updateProfile:', profileDataWithSpecs)
      // @ts-ignore - Ignoring strict type check for partial updates
      await updateProfile(profileDataWithSpecs, profileImage || undefined)
      console.log('✅ [ProfileEditModal] Perfil actualizado exitosamente. Llamando a onClose().')
      onClose()
    } catch (error) {
      console.error("❌ [ProfileEditModal] Error al guardar perfil:", error)
      toast({
        title: "Error",
        ...
        <DialogContent className="w-[95vw] max-w-[480px] p-0 overflow-hidden bg-black/60 backdrop-blur-2xl border-white/10 text-white shadow-2xl rounded-3xl">
          ...
          <div className="absolute top-0 left-0 right-0 z-20 px-6 py-4 flex items-center justify-between bg-gradient-to-b from-black/80 to-transparent">
            ...
            <div className="flex items-center gap-2">
              {/* Botón Guardar flotante superior */}
              <Button
