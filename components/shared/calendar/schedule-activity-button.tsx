...
import { useActivitiesStore } from '@/hooks/shared/use-activities-store'

interface ScheduleActivityButtonProps {
  activityId: number
  ...
  const handleSchedule = async () => {
    ...
    try {
      ...
      const eventData: any = {
        ...
        client_id: user.id,
        title: activityTitle,
        description: `Sesión programada de ${activityTitle}`,
      ...
      const { data: newEvent, error } = await supabase.from("calendar_events").insert([eventData]).select().single()
      ...
      if (newEvent) {
        try {
          const syncResponse = await fetch('/api/google/calendar/sync-event', {
            method: 'POST',
          ...
          if (syncResult.success && syncResult.synced) {
            console.log('✅ Evento sincronizado automáticamente con Google Calendar')
          }
        } catch (syncError) {
          console.log('ℹ️ No se pudo sincronizar con Google Calendar (normal si no está conectado)')
        }
      }
      ...
      toast({
        ...
        description: `Has programado ${activityTitle} para el ${new Date(selectedDate).toLocaleDateString()} a las ${selectedTime}`,
      })
      ...
      toast({
        title: "Error",
        description: error.message || "No se pudo programar la actividad. Por favor, intenta de nuevo más tarde.",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
        ...
        <div className="fixed inset-0 bg-black/80 z-40 flex items-center justify-center p-4">
          <div className="bg-[#1E1E1E] rounded-xl w-full max-w-md overflow-hidden">
            <div className="p-5 border-b border-gray-800 flex justify-between items-center">
              <h3 className="text-lg font-medium text-white">Programar Actividad</h3>
            ...
            <div className="p-5">
              <div className="mb-4">
                <h4 className="text-lg font-medium text-white mb-2">{activityTitle}</h4>
                <p className="text-gray-300 mb-4">
                  Selecciona la fecha y hora para programar esta actividad en tu calendario.
                </p>
            ...
            <div className="p-5 border-t border-gray-800 flex justify-end space-x-3">
              ...
              <Button
                ...
                disabled={isLoading}
              >
                {isLoading ? "Programando..." : "Confirmar"}
              </Button>
            </div>
          </div>
        </div>
