            ...
            <div className="space-y-4 animate-in fade-in duration-300">
                <div className="p-3.5 bg-white/[0.03] rounded-2xl border border-white/10 shadow-2xl">
                    <div className="flex items-center gap-2 mb-4">
                         <ListOrdered className="h-4 w-4 text-[#FF7939]" />
                         <span className="text-[10px] font-black text-zinc-400 uppercase tracking-widest italic">Pasos de Preparación</span>
                    </div>

                    <div className="flex flex-col gap-3 mb-6 bg-zinc-950/40 p-3 rounded-xl border border-white/5 shadow-inner">
                        <div className="space-y-1.5">
                        ...
                        <Button 
                            className="h-10 w-full bg-[#FF7939] hover:bg-[#FF6B35] text-white rounded-lg shadow-lg active:scale-95 text-[10px] font-black uppercase italic tracking-widest"
                            onClick={() => {
                    ...
                    <div className="space-y-3 max-h-[350px] overflow-y-auto no-scrollbar py-1">
                        ...
                        {formState.receta?.split(';').filter(Boolean).map((step, idx) => (
                            <div key={idx} className="flex gap-4 bg-zinc-900/60 p-4 rounded-xl border border-white/5 shadow-inner group hover:border-[#FF7939]/30 transition-all relative">
                                ...
                                <p className="text-[10px] font-bold text-zinc-300 leading-relaxed pt-1.5 pr-6">{step}</p>
                                <button
                                    onClick={() => {
            ...
            <div className="space-y-4 animate-in fade-in duration-300">
                <div className="p-3.5 bg-white/[0.03] rounded-2xl border border-white/10 shadow-2xl">
                    <div className="flex items-center gap-2 mb-3">
                         ...
                         <span className="text-[10px] font-black text-zinc-400 uppercase tracking-widest italic">Lista de Ingredientes</span>
                    </div>
                    ...
                    <div className="flex flex-col gap-3 mb-6 bg-zinc-950/40 p-3 rounded-xl border border-white/5 shadow-inner">
                        ...
                        <div className="flex gap-2 items-end pt-1">
                            ...
                            <div className="flex-1">
                                ...
                                {/* Invisible input for logic compatibility if needed */}
                                <input type="hidden" id="ing-unit" defaultValue="g" />
                            </div>
                            <Button 
                                className="h-9 px-6 bg-[#FF7939] hover:bg-[#FF6B35] text-white rounded-lg shadow-lg active:scale-95 text-[10px] font-black uppercase italic tracking-widest"
                                onClick={() => {
                    ...
                    <div className="space-y-2 max-h-[220px] overflow-y-auto no-scrollbar py-1">
                        ...
                        {formState.ingredientes?.split(';').filter(Boolean).map((ing, idx) => {
                            // Intentar extraer cantidad, unidad y nombre (Ej: 200g Pollo o 1unidad Manzana)
                            const match = ing.match(/^(\d+)([a-zA-ZáéíóúÁÉÍÓÚñÑ]+)?\s+(.+)$/)
                            
                            return (
                                <div key={idx} className="flex items-center justify-between bg-zinc-900/60 px-4 py-2.5 rounded-xl border border-white/5 shadow-inner group hover:border-[#FF7939]/30 transition-all">
