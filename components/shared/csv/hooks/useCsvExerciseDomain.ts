...
export function useCsvExerciseDomain({
    ...
    const handleEditExercise = useCallback((exercise: ExerciseData, index: number) => {
        ...
        const exerciseData: any = {
            ...
            duracion_min: exercise.duracion_min ?? exercise['Duración (min)'] ?? exercise.duracion ?? '',
            tipo_ejercicio: normalizeExerciseType(exercise.tipo_ejercicio || exercise['Tipo de Ejercicio'] || ''),
            nivel_intensidad: exercise.intensidad ?? exercise['Nivel de Intensidad'] ?? '',
            equipo_necesario: exercise.equipo_necesario ?? exercise['Equipo Necesario'] ?? '',
            detalle_series: exercise.detalle_series ?? exercise['Detalle de Series (peso-repeticiones-series)'] ?? '',
            partes_cuerpo: exercise.body_parts ?? exercise['Partes del Cuerpo'] ?? '',
    ...
    const addManualExercise = useCallback(() => {
        ...
        let item: any = {
            ...
            duracion_min: manualForm.duracion_min, // Canonical key
            segundos: manualForm.segundos,
