...
export function WorkshopSimpleScheduler({ sessions, onSessionsChange }: WorkshopSimpleSchedulerProps) {
  ...
  const handleAddTimeSlot = () => {
    ...
    if (editingPrimary) {
      setPrimaryTimeSlots(prev => [...prev, newSlot])
    } else {
      setSecondaryTimeSlots(prev => [...prev, newSlot])
    }
  ...
  const handleRemoveTimeSlot = (id: string, isPrimary: boolean) => {
    if (isPrimary) {
      setPrimaryTimeSlots(prev => prev.filter(slot => slot.id !== id))
    } else {
      setSecondaryTimeSlots(prev => prev.filter(slot => slot.id !== id))
    }
  }
  ...
  const getTopicSummary = () => {
    if (!finishedTopic) return { primaryDays: 0, secondaryDays: 0, totalHours: 0 }
    
    const primaryDates = new Set<string>()
    const secondaryDates = new Set<string>()
    let totalHours = 0
    
    // Agregar fechas y horas de horarios
    finishedTopic.timeSlots.forEach(slot => {
      slot.dates.forEach(date => primaryDates.add(date))
      totalHours += slot.duration * slot.dates.length
    })
    
    return {
      primaryDays: primaryDates.size,
      secondaryDays: 0,
  ...
  const handleFinishTopic = () => {
    if (topicTitle.trim() && (primaryTimeSlots.length > 0 || secondaryTimeSlots.length > 0)) {
      // Crear sesiones para enviar al padre
      const newSessions: WorkshopSession[] = []
      
      // Agregar sesiones principales
  ...
  const handleEditGroupedTopic = (topicTitle: string) => {
    ...
    topicSessions.forEach(session => {
      ...
      if (session.isPrimary) {
        if (primarySlots.has(slotKey)) {
          // Agregar fecha al slot existente
          primarySlots.get(slotKey)!.dates.push(session.date)
        } else {
          primarySlots.set(slotKey, slot)
        }
      } else {
        if (secondarySlots.has(slotKey)) {
          // Agregar fecha al slot existente
          secondarySlots.get(slotKey)!.dates.push(session.date)
        } else {
          secondarySlots.set(slotKey, slot)
        }
      }
    })
    
    // Convertir maps a arrays y ordenar fechas
    const primaryTimeSlotsArray = Array.from(primarySlots.values()).map(slot => ({
      ...slot,
      dates: slot.dates.sort()
    }))
    
    const secondaryTimeSlotsArray = Array.from(secondarySlots.values()).map(slot => ({
      ...slot,
      dates: slot.dates.sort()
    }))
        ...
        <div className="space-y-6">
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <div className={`w-1 h-6 rounded ${editingPrimary ? 'bg-[#FF7939]' : 'bg-gray-600'}`}></div>
                <Clock className="w-5 h-5 text-[#FF7939]" />
                <h3 className="text-lg font-medium text-white">Horario Principal</h3>
              </div>
              {editingPrimary && (
            ...
            <div className="space-y-1">
              {primaryTimeSlots.map(slot => (
                ...
                >
                  ...
                  <button
                    onClick={() => handleRemoveTimeSlot(slot.id, true)}
          ...
          <div className="flex items-center justify-between py-3 border-t border-[#3A3A3A]">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-medium text-white">Horario Secundario</h3>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
              <input 
                ...
                checked={bisEnabled}
                onChange={(e) => setBisEnabled(e.target.checked)}
              />
          ...
          {bisEnabled && (
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <div className={`w-1 h-6 rounded ${!editingPrimary ? 'bg-orange-600' : 'bg-gray-600'}`}></div>
                  <Clock className="w-5 h-5 text-orange-500" />
                  <h3 className="text-lg font-medium text-white">Horario Secundario</h3>
                </div>
                {!editingPrimary && (
                  <span className="text-xs text-orange-600 font-medium">● Editando</span>
                )}
              </div>
                ...
                <div className="space-y-2 mb-3">
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      value={currentStartTime}
                      onChange={(e) => setCurrentStartTime(e.target.value)}
                      className="flex-1 px-2 py-1 bg-[#1A1A1A] border-b border-[#3A3A3A] text-white text-xs focus:border-orange-500 focus:outline-none"
                    />
                    <span className="text-gray-500 text-xs">-</span>
                    <input
                      type="time"
                    ...
                    <button
                      onClick={handleAddTimeSlot}
                      disabled={selectedDates.size === 0}
                      className="px-3 py-1 bg-orange-600 hover:bg-orange-700 disabled:bg-gray-600 disabled:cursor-not-allowed text-white rounded text-xs font-medium transition-colors flex items-center gap-1"
                    >
                      <Plus className="w-3 h-3" />
                      Agregar
                    </button>
              ...
              {/* Lista de time slots secundarios - Compacto */}
              <div className="space-y-1">
                {secondaryTimeSlots.map(slot => (
                  <div
                    key={slot.id}
                    className="flex items-center justify-between w-full py-2 px-1 hover:bg-gray-800/30 transition-colors border-l-2 border-orange-600"
                  >
                    <div className="flex items-center gap-3 flex-1">
                      <div className="flex items-center gap-3 text-sm">
                        {/* Horario */}
                        ...
                        <div className="flex gap-1">
                          {slot.dates.map(date => (
                            <span
                              key={date}
                              className="px-1.5 py-0.5 bg-orange-600 text-white text-xs rounded"
                            >
                              {formatDateShort(date)}
                            </span>
                          ))}
                        ...
                        <span className="text-orange-400 text-xs">sec</span>
                      </div>
                    </div>
                    <button
                      onClick={() => handleRemoveTimeSlot(slot.id, false)}
                      className="p-1 hover:bg-gray-800 rounded transition-colors ml-2"
                    >
                      <Trash2 className="w-3 h-3 text-red-400" />
                    </button>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-2 text-xs text-gray-300">
                  ...
                  <span className="text-gray-500">|</span>
                  <span>{getTopicSummary().primaryDays} orig</span>
                  <span className="text-gray-500">|</span>
                  <span>{getTopicSummary().secondaryDays} sec</span>
                </div>
              </div>
      ...
      {!showSummary && (
        <div className="pt-4 space-y-2">
          <button
            ...
            disabled={primaryTimeSlots.length === 0 || !topicTitle.trim()}
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-3 text-xs text-gray-300">
                      ...
                      <span className="text-gray-500">|</span>
                      <span>{group.originalCount} orig</span>
                      <span className="text-gray-500">|</span>
                      <span>{group.secondaryCount} sec</span>
                    </div>
                  </div>
