...
export interface SimpleExerciseData {
  ...
  'Descripción': string
  'Duración (min)': string | number
  'Tipo de Ejercicio': string
  'Nivel de Intensidad': string
  'Equipo Necesario': string
  'Detalle de Series (peso-repeticiones-series)': string
  'Partes del Cuerpo': string
  'Calorías': string | number
}
...
/**
 * Valida que el CSV tenga las columnas requeridas para el formato simple
 */
export function validateSimpleCSVHeaders(headers: string[], productCategory?: 'fitness' | 'nutricion'): boolean {
  // console.log('🔍 Validando headers del CSV simple:', headers)
  
  // Headers para nutrición
  ...
  // Detectar automáticamente si es nutrición o fitness
  const hasNutritionHeaders = nutritionHeaders.filter(h => 
    headers.some(header => header.trim().toLowerCase() === h.toLowerCase())
  ).length >= 4

  const requiredHeaders = hasNutritionHeaders || productCategory === 'nutricion' 
    ? nutritionHeaders 
    : fitnessHeaders

  const minRequiredHeaders = hasNutritionHeaders || productCategory === 'nutricion' ? 4 : 6

  // Verificar que al menos los headers mínimos requeridos estén presentes
  ...
  // Si encontramos al menos los headers mínimos, consideramos válido
  if (foundHeaders.length >= minRequiredHeaders) {
    // console.log('✅ CSV válido para formato simple')
    return true
  }
  ...
  console.error('❌ Headers faltantes en CSV simple:', missingHeaders)
