-- 1. Modify progreso_diario_actividad to have the 4 unified columns
ALTER TABLE public.progreso_diario_actividad
ADD COLUMN IF NOT EXISTS fitness_items JSONB DEFAULT '{"completados": 0, "objetivo": 0}'::jsonb,
ADD COLUMN IF NOT EXISTS fitness_minutos JSONB DEFAULT '{"completados": 0, "objetivo": 0}'::jsonb,
ADD COLUMN IF NOT EXISTS nutricion_items JSONB DEFAULT '{"completados": 0, "objetivo": 0}'::jsonb,
ADD COLUMN IF NOT EXISTS nutricion_calorias JSONB DEFAULT '{"completados": 0, "objetivo": 0}'::jsonb,
ADD COLUMN IF NOT EXISTS nutricion_macros JSONB DEFAULT '{}'::jsonb;

-- 2. Create Unique Constraint for (cliente_id, fecha)
DROP INDEX IF EXISTS public.idx_pda_enrollment_fecha;
DROP INDEX IF EXISTS public.idx_progreso_actividad_dia;
CREATE UNIQUE INDEX IF NOT EXISTS idx_pda_cliente_fecha ON public.progreso_diario_actividad (cliente_id, fecha);
...
BEGIN
    ...
    IF TG_TABLE_NAME = 'progreso_cliente_nutricion' THEN
        -- NUTRITION (Using k, m, p, c, g keys)
