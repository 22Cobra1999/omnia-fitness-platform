-- Fix Calendar Move Sync and Minutes Calculation
-- 1. Correct Trigger Function to include targets and handle date changes
CREATE OR REPLACE FUNCTION public.update_daily_progress_from_program()
RETURNS TRIGGER AS $$
DECLARE
    v_total_items INTEGER;
    v_completed_items INTEGER;
    v_enrollment_id UUID; -- Corrected to UUID
    v_area TEXT;
...
BEGIN
    ...
    -- If the date changed, the OLD PDA entry for this enrollment is now stale.
    ...
    IF TG_TABLE_NAME = 'progreso_cliente_nutricion' THEN
        ...
        IF jsonb_typeof(NEW.macros) = 'object' THEN
            -- Calculate Objective (all items in macros)
            SELECT 
            ...
            -- Calculate Completed (only items in ejercicios_completados)
            FOR v_row IN 
    ...
    ELSE
        ...
        -- Calculate Objectives from minutes/minutos_json
        ...
        ELSIF jsonb_typeof(NEW.minutos_json) = 'object' THEN
        ...
        ELSIF jsonb_typeof(NEW.calorias_json) = 'object' THEN
        ...
        IF v_completed_items > 0 THEN
            ...
            LEFT JOIN jsonb_each_text(COALESCE(NEW.minutos, NEW.minutos_json, '{}'::jsonb)) m ON m.key = keys.k
            LEFT JOIN jsonb_each_text(COALESCE(NEW.calorias, NEW.calorias_json, '{}'::jsonb)) c ON c.key = keys.k;
        END IF;
...
-- 2. Ensure triggers are attached
