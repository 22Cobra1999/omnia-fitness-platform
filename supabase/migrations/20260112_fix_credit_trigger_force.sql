-- FORCE update of credit trigger logic
...
DROP FUNCTION IF EXISTS deduct_client_credits CASCADE;

-- 2. Re-create main function
CREATE FUNCTION manage_credits_on_participation()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    -- CASO 1: Nueva Solicitud (INSERT) - Solo si payment_status es 'credit_deduction'
...
-- 4. Re-create RPC for partial deduction
CREATE FUNCTION deduct_client_credits(p_client_id UUID, p_amount INT, p_event_id UUID, p_description TEXT)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
