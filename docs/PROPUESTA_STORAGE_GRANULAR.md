# 🎯 Propuesta: Tracking Granular de Archivos en Storage

## ⚠️ Problema Actual

La tabla `storage_usage` actual agrupa **todo** por concepto (video/image/pdf) y luego lista **todas** las actividades del coach, incluso si no usan ese archivo específico.
...
## 🎯 Solución Recomendada: Opción A - Nueva Tabla `storage_files`

Crear una tabla que rastrea **cada archivo individual** y dónde se usa.
...
CREATE TABLE storage_files (
  ...
  activity_id BIGINT NOT NULL REFERENCES activities(id), -- Qué actividad lo usa
  
  -- Metadata temporal
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
  
  -- Combinación única: mismo archivo puede estar en múltiples lugares
...
-- Video 1 usado en actividad 90 (otra referencia - puede pasar si está duplicado)
INSERT INTO storage_files VALUES
...
-- Ver qué actividades usan un archivo específico
SELECT 
  sf.activity_id,
  a.title as actividad_nombre,
  sf.file_name
...
-- Ver archivos huérfanos (archivos que no están en ninguna actividad activa)
SELECT 
  sf.*
FROM storage_files sf
...
## 🔄 Solución Alternativa: Opción B - Mejorar `products` en `storage_usage`

Si no quieres crear una nueva tabla, puedes **arreglar** el cálculo de `products` en la tabla actual.

### Cambios Necesarios:

1. **Para Videos:** Ya tienes `videoProductMap` que mapea cada `bunny_video_id` a sus `activity_ids`. Usa eso.
2. **Para Imágenes:** Consulta `activity_media` donde `image_url IS NOT NULL` y agrupa por archivo.
3. **Para PDFs:** Similar a imágenes.
...
// Para imágenes, necesitas similar mapeo:
const imageProductMap: Record<string, number[]> = {} // imageUrl -> [activity_ids]

// Obtener imágenes con sus actividades
...
if (imageGb > 0) {
  ...
  storageData.push({
    ...
    products: uniqueImageActivityIds  // ✅ Solo actividades que usan imágenes
  })
}
```

**Limitación:** Esto sigue agregando **todas** las actividades, no te dice qué archivo usa qué actividad.

---

## 🎯 Comparación

| Aspecto | Opción A: `storage_files` | Opción B: Mejorar `storage_usage` |
|---------|---------------------------|-----------------------------------|
| **Granularidad** | ✅ Por archivo individual | ❌ Solo por concepto agregado |
| **Rastreabilidad** | ✅ Sabes qué archivo usa qué actividad | ❌ Solo listas todas las actividades |
| **Archivos huérfanos** | ✅ Puedes encontrar archivos no usados | ❌ No puedes |
| **Deduplicación** | ✅ Cuentas una vez por archivo único | ⚠️ Requiere lógica adicional |
| **Migración** | ⚠️ Nueva tabla, migrar datos históricos | ✅ Solo cambiar código |
| **Compatibilidad** | ✅ Mantener `storage_usage` como vista | ✅ Compatible con existente |
| **Performance** | ⚠️ Más filas = más datos | ✅ Pocas filas |
...
2. Permite encontrar archivos huérfanos
...
5. **Agregar cleanup automático:**
   - Script que elimine registros de archivos eliminados

---

¿Quieres que implemente la Opción A o prefieres la Opción B?
